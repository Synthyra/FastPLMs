...
`Synthyra/ANKH2_large` packages the `ElnaggarLab/ankh2-ext2` checkpoint with
the FastPLMs runtime for Hugging Face Transformers. It accepts amino-acid
sequences tokenized for encoder or sequence-to-sequence use.
...
`dist/hub/ANKH2_large` path. Pass `local_files_only=True`.
...
`Synthyra/ANKH2_large` contains the complete encoder-decoder checkpoint.
...
`Synthyra/ANKH2_large` contains the complete ANKH encoder-decoder checkpoint.
...
- FastPLMs checkpoint: `Synthyra/ANKH2_large`
