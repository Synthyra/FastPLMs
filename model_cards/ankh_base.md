...
`Synthyra/ANKH_base` packages the `ElnaggarLab/ankh-base` checkpoint with the
...
python -m pip install -r \
  "https://huggingface.co/Synthyra/ANKH_base/resolve/main/requirements.txt"
```
...
`dist/hub/ANKH_base` path. Pass `local_files_only=True`.
...
`Synthyra/ANKH_base` contains the complete encoder-decoder checkpoint.
...
`Synthyra/ANKH_base` contains the complete ANKH encoder-decoder checkpoint.
...
- FastPLMs checkpoint: `Synthyra/ANKH_base`
...
- Official checkpoint: `ElnaggarLab/ankh-base`
