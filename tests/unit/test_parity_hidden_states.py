...
def test_live_parity_accepts_layer_stacked_hidden_states() -> None:
    ...
    H = torch.arange(3 * 2 * 4 * 5).reshape(3, 2, 4, 5)  # (n_layers=3, b=2, l=4, d=5)
    output = SimpleNamespace(hidden_states=H, last_hidden_state=None)

    layers = _hidden_state_tuple(output)  # 3 * (b=2, l=4, d=5)
    assert len(layers) == 3
...
def test_live_parity_pooling_ignores_nonfinite_padding() -> None:
    candidate = torch.tensor(  # (b=1, l=2, d=2)
        [[[1.0, 2.0], [float("nan"), float("nan")]]]
    )
    official = torch.tensor(  # (b=1, l=2, d=2)
        [[[1.0, 2.0], [float("nan"), float("nan")]]]
    )
    residue_mask = torch.tensor([[True, False]])  # (b=1, l=2)
