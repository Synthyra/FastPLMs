...
class PairCollator:
    ...
    def __call__(self, batch: list[tuple[str, str, float | int]]) -> dict[str, torch.Tensor]:
        ...
        tokenized = self.tokenizer(
            seqs_a, seqs_b, padding="longest", pad_to_multiple_of=8, return_tensors="pt"
        )
        return {
...
class SequenceCollator:
    ...
    def __call__(self, batch: list[tuple[str, float | int]]) -> dict[str, torch.Tensor]:
        ...
        labels = labels.float() if self.regression else labels.long()
        tokenized = self.tokenizer(
            seqs, padding="longest", pad_to_multiple_of=8, return_tensors="pt"
        )
        return {
            "input_ids": tokenized["input_ids"],
            "attention_mask": tokenized["attention_mask"],
...
def initialize_model(
    model_name: str, num_labels: int, use_lora: bool = True, lora_config: Any = None
):
    """
...
def compute_metrics_regression(p: EvalPrediction) -> dict[str, float]:
    ...
    return {"spearman_correlation": correlation, "p_value": p_value}
...
def plot_regression_results(
    preds: np.ndarray, labels: np.ndarray, task_name: str = "Regression"
) -> float:
    """
...
def plot_classification_results(
    trainer: Trainer, test_dataset: Any, task_name: str = "Classification"
) -> float:
    """
...
def train_regression_model(
    ...
    # Load datasets
    train_data = load_dataset("Synthyra/ProteinProteinAffinity", split="train").filter(
        _filter_pair_by_length
    )
    valid_data = load_dataset("Synthyra/AffinityBenchmarkv5.5", split="train").filter(
        _filter_pair_by_length
    )
    test_data = load_dataset("Synthyra/haddock_benchmark", split="train").filter(
        _filter_pair_by_length
    )
    ...
    correlation = plot_regression_results(
        preds.flatten(), labels.flatten(), "Protein-Protein Affinity"
    )
