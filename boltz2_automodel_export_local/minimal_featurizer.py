...
def _build_template(
    ...
    for res_idx, aa in enumerate(sequence):
        ...
        for local_idx, atom_name in enumerate(residue_atoms):
            ...
            atom_pos = _default_atom_offset(atom_name, local_idx)
...
def _center_atoms_per_residue(
    atom_positions: torch.Tensor,
    atom_residue_index: List[int],
    num_residues: int,
) -> torch.Tensor:
    centered = atom_positions.clone()
    ...
    for residue_idx in range(num_residues):
        ...
        residue_coords = centered[residue_mask]
        residue_center = residue_coords.mean(dim=0, keepdim=True)
        centered[residue_mask] = residue_coords - residue_center
    return centered
...
def build_boltz2_features(
    ...
    atom_positions = _center_atoms_per_residue(
    ...
    assert "SINGLE" in const.bond_type_ids
    assert "COVALENT" in const.bond_type_ids
    single_bond_id = const.bond_type_ids["SINGLE"] + 1
    covalent_bond_id = const.bond_type_ids["COVALENT"] + 1
    for idx in range(num_tokens):
        token_bonds[idx, idx] = 1.0
        type_bonds[idx, idx] = single_bond_id
