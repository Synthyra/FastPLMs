...
class FlexEsmConfig(PretrainedConfig):
    model_type = "flex_esm"
    def __init__(
        self,
        vocab_size=None,
...
class FlexEsmPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """
    config_class = FlexEsmConfig
    base_model_prefix = "flexesm"
...
class FlexEsmModel(FlexEsmPreTrainedModel):
    ...
    def forward(
        ...
        if output_attentions:
            ...
            seqs = (input_ids == self.cls_id).cumsum(0)

            def doc_mask_mod(b, h, q_idx, kv_idx):
                doc_mask = seqs[q_idx] == seqs[kv_idx]
                return doc_mask
            
            S = len(input_ids)
            extended_attention_mask = create_block_mask(doc_mask_mod, None, None, S, S)
...
class FlexEsmForMaskedLM(FlexEsmPreTrainedModel):
    _tied_weights_keys = ["lm_head.decoder.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.esm = FlexEsmModel(config, add_pooling_layer=False)
...
class FlexEsmForSequenceClassification(FlexEsmPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class FlexEsmForTokenClassification(FlexEsmPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.esm = FlexEsmModel(config, add_pooling_layer=False)
...
if __name__ == "__main__":
    """
    Test the hidden state differences between the FlexEsmModel and the HF EsmModel.
    ...
    In Pytorch 2.5+ (and linux kernel), this implementation is very flex and uses less memory than the HF implementation.
    """
    import random
    ...
    for model_path in model_paths:
        ...
        config = FlexEsmConfig.from_pretrained(model_path)
        config.cls_id = tokenizer.cls_token_id
        flex_model = FlexEsmForMaskedLM(config).from_pretrained(model_path).to(device)
        ...
        for _ in range(seq_count):
            ...
            flex_tokens = tokenizer(example_seq, return_tensors="pt").input_ids.to(device)
            flex_output = flex_model(flex_tokens, output_hidden_states=True).hidden_states[-1].detach().cpu()
            ...
            for i, atol in enumerate(tolerances):
                if torch.allclose(flex_output, model_output, atol=atol):
                    counts[i] += 1
