...
Before submitting a PR for a new model, ensure:

1. `test_automodel_loads` and `test_automodel_forward_pass` pass
2. `test_backend_consistency` passes for all available backends
3. `test_nan_stability` passes
4. `test_batch_single_match` passes (tokenizer-mode models)
5. `test_weight_compliance` passes (if compliance deps are available)
6. `test_forward_compliance` passes (if compliance deps are available)
