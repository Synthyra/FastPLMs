...
def gpu_budget(category: str, stage: str, budget_hours: float) -> Iterator[float]:
    ...
    try:
        ...
        entries.append({"category": category, "stage": stage, "started": started, "seconds": time.time() - started})
...
def fetch_pilot_artifacts() -> None:
    ...
    environment = {**os.environ, **{name: value for name, value in dotenv_values(MODAL_CREDENTIALS).items() if value}}
    PILOT_DIR.mkdir(parents=True, exist_ok=True)
    for remote, local in PILOT_FILES.items():
        subprocess.run(
            [str(MODAL_CLI), "volume", "get", "--force", "fastplms-confidence-pilot", remote, str(PILOT_DIR / local)],
            check=True,
            env=environment,
        )
        log(f"copied {remote}")
...
def stage_smoke(check: str, model_id: str) -> None:
    ...
    with gpu_budget("smoke", f"smoke-{check}-{model_id}", SMOKE_BUDGET_HOURS):
        if check == "throughput":
            # Long targets come from the unused split, so sizing the evaluation never touches the test set.
            long_targets = [target for target in split_targets("unused") if target["variant"] == "long"]
            chosen = closest_targets(train, (128, 384, 768, 1024), 768) + closest_targets(long_targets, (1536, 2048), 0)
            ...
            sampler = TargetSampler(train, OnlineTrainingConfig(model_id=model_id).monomer_fraction, seed=RATE_SAMPLER_SEED)
            result = training_rate(model_id, POOL_DIR, [sampler.draw() for _ in range(RATE_TARGETS + 1)], log)
        elif check == "parity":
            chosen = [
                next(target for target in short if int(target["num_chains"]) == 1),
                next(target for target in short if int(target["num_chains"]) == 2 and target["sequences"][0] == target["sequences"][1]),
                next(target for target in short if int(target["num_chains"]) == 2 and target["sequences"][0] != target["sequences"][1]),
            ]
...
def stage_train(args: argparse.Namespace) -> None:
    ...
    category, budget = ("smoke", SMOKE_BUDGET_HOURS) if args.probe else (f"model-{args.model}", MODEL_BUDGET_HOURS)
    with gpu_budget(category, f"train-{args.run}", budget) as remaining_seconds:
        ...
        deadline_seconds = remaining_seconds - (0.0 if args.probe else EVALUATION_RESERVE_HOURS * 3600)
        report = train_online(
            model, POOL_DIR, split_targets("train"), split_targets("validation"), RUNS_DIR / args.model / args.run, config, log, deadline_seconds
        )
    log(json.dumps(report))


def reference_model() -> object:
    """Production `esmfold2` with its release confidence head on the CPU, pinned by the model registry."""
    import torch
    ...
    config = ESMFold2Config.from_pretrained(spec.fast.repo_id, revision=spec.fast.revision, attn_implementation="sdpa", esmc_precision="bf16")
...
def evaluation_scope(
    ...
    Only the test split is the pre-registered evaluation. Any other split is a dry run of the same
    code on a few targets; it is charged to the smoke budget and written apart from test results.
    """
...
def stage_evaluate(model_id: str, run: str, split: str, limit: int | None) -> None:
    from .cache import load_folding_model
    from .rollouts import use_fast_folding_kernels
    ...
    head_files = {
        "v2": run_dir / report["selected_checkpoint"],
        "pilot": PILOT_DIR / f"{model_id}-head.safetensors",
        "donor": None,
    }
    targets, category, budget, output_root = evaluation_scope(split, limit, f"model-{model_id}", MODEL_BUDGET_HOURS)
    output_dir = output_root / model_id
    with gpu_budget(category, f"evaluate-{run}-{split}", budget):
        model = load_folding_model(model_id)
        use_fast_folding_kernels(model)
        records = fold_and_score(model, POOL_DIR, targets, load_heads(model_id, head_files), output_dir / "records.json", log)
    summary = summarize(records, list(head_files))
    write_json(output_dir / "summary.json", {"run": run, "head_files": {name: str(path) for name, path in head_files.items()}, **summary})
    log(json.dumps(summary, indent=2)[:4000])


def stage_reference(split: str, limit: int | None) -> None:
    from .rollouts import use_fast_folding_kernels
    from .test_evaluation import fold_and_score, summarize

    targets, category, budget, output_root = evaluation_scope(split, limit, "reference", REFERENCE_BUDGET_HOURS)
    output_dir = output_root / "esmfold2"
    with gpu_budget(category, f"reference-esmfold2-{split}", budget):
        model = reference_model().to("cuda")  # type: ignore[attr-defined]
        use_fast_folding_kernels(model)
        records = fold_and_score(model, POOL_DIR, targets, None, output_dir / "records.json", log)
    summary = summarize(records, ["production"])
    write_json(output_dir / "summary.json", summary)
    log(json.dumps(summary, indent=2)[:4000])


def stage_acceptance(model_id: str) -> None:
    ...
    reference_records = json.loads((evaluation / "esmfold2" / "records.json").read_text(encoding="utf-8"))
    ...
    write_json(evaluation / f"acceptance-{model_id}.json", {**result, "production_agreement": agreement, "estimates": estimates})
...
def main() -> None:
    ...
    # These checkpoints fold single sequences by design; the per-chain notice would fill day-long logs.
    ...
    splits_parser = stages.add_parser("splits", help="cluster chains and assign train, validation, and test")
    ...
    smoke_parser.add_argument("check", choices=("throughput", "kernels", "parity", "overfit", "training-rate"))
    ...
    train_parser.add_argument("--probe", action="store_true", help="charge the smoke budget instead of the model budget")
    evaluate_parser = stages.add_parser("evaluate", help="score the v2, pilot, and donor heads on the test set")
    ...
    evaluate_parser.add_argument("--split", default="test", choices=("test", "validation"), help="validation runs a dry run")
    evaluate_parser.add_argument("--limit", type=int, help="evaluate only the first targets")
    reference_parser = stages.add_parser("reference", help="score production esmfold2 on the test set")
    reference_parser.add_argument("--split", default="test", choices=("test", "validation"), help="validation runs a dry run")
    reference_parser.add_argument("--limit", type=int, help="evaluate only the first targets")
    stages.add_parser("download-reference", help="cache the production checkpoint without using the GPU")
    acceptance_parser = stages.add_parser("acceptance", help="apply the pre-registered gates to test records")
    acceptance_parser.add_argument("--model", required=True, choices=("esmfold2_300", "esmfold2_600"))
    ...
    stages_by_name: dict[str, Callable[[], None]] = {
        ...
        "evaluate": lambda: stage_evaluate(args.model, args.run, args.split, args.limit),
        "reference": lambda: stage_reference(args.split, args.limit),
        "download-reference": stage_download_reference,
        "acceptance": lambda: stage_acceptance(args.model),
    }
