...
SYNCED_PREFIXES = ("src/", "tools/", "tests/", "requirements/", "pytest.ini", "ruff.toml", "mypy.ini")
...
def ssh_command(host: str, identity: Path, remote_command: str) -> list[str]:
    return ["ssh", "-i", str(identity), "-o", "BatchMode=yes", "-o", "ServerAliveInterval=60", host, remote_command]
...
def start(host: str, identity: Path, job: str, stage_arguments: list[str], after: str | None) -> None:
    ...
    if after is None:
        ...
        job_command = (
            f"cd ~/{REMOTE_REPO} && until tail -n 1 {previous_log} 2>/dev/null | grep -q '^\\[exit status'; do sleep 60; done; "
            f"if tail -n 1 {previous_log} | grep -q '^\\[exit status 0\\]'; then {run_stage}; "
            f'else echo "[exit status 1] {after} did not exit cleanly" | tee -a {REMOTE_LOGS}/{job}.log; fi'
        )
    command = (
...
def wait(host: str, identity: Path, job: str, tail_bytes: int) -> None:
    """Block until a job's log ends with its exit status, then print the log tail without progress bars."""
    log_path = f"{REMOTE_LOGS}/{job}.log"
    command = (
        f"until tail -n 1 {log_path} 2>/dev/null | grep -q '^\\[exit status'; do sleep 15; done; "
...
def main() -> None:
    ...
    actions.add_parser("push-secrets", help="deliver HF_TOKEN and WANDB_API_KEY without printing them")
    start_parser = actions.add_parser("start", help="run a host stage in a detached tmux job")
    start_parser.add_argument("--after", help="run only after this job exits with status 0; give it before the job name")
