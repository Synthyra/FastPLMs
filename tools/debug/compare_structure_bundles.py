...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    actual = load_file(arguments.actual, device="cpu")
    expected = load_file(arguments.expected, device="cpu")
    keys = sorted(
        key
    ...
    for key in keys:
        ...
        if shape_matches and X.numel() > 0:
            ...
            if unequal_count:
                indices = unequal.nonzero(as_tuple=False)
                if arguments.largest_first:
                    errors = (X.float() - X_ref.float()).abs()[unequal]
                    order = torch.argsort(errors, descending=True)
